...
namespace BTool
{
    ...
    class LastTaskPool : private boost::noncopyable
    {
...
#pragma endregion
        ...
        public:
            ...
            LastTaskList(size_t max_task_count = 0)
...
#  endif
            ...
            {
                writeLock locker(m_tasks_mtx);
                auto iter = m_wait_props.find(prop);
                if (iter != m_wait_props.end())
                {
                    auto list_iter = m_wait_tasks.begin();
                    std::advance(list_iter, iter->second);

                    while (list_iter != m_wait_tasks.end())
                    {
                        auto next_list_iter = m_wait_props.find((*list_iter)->get_prop_type());
                        if (next_list_iter != m_wait_props.end())
                            next_list_iter->second--;
                        list_iter++;
                    }
                    m_wait_props.erase(prop);
                    m_wait_tasks.erase(list_iter++);
                }
            ...
            void pop_task()
            {
...

                {
                    ...
                    {
                        PropType prop_type = (*iter)->get_prop_type();
                        ...
                        if(!bFind)
                        {
                            ...
                            pop_iter = iter;
                            pop_task_ptr = *iter;
                            pop_prop_type = prop_type;
                        }
                        else
                        {
                            auto next_list_iter = m_wait_props.find((*iter)->get_prop_type());
                            if (next_list_iter != m_wait_props.end())
                                next_list_iter->second--;
                        }
                        iter++;
                    ...
                    if(bFind)
                    {
                        m_wait_props.erase(pop_prop_type);
                        m_wait_tasks.erase(pop_iter);
                        m_cv_not_full.notify_one();
                    }
                }
...
#if (defined __linux)
            // 新增任务至队列
            bool add_task_tolist(const TaskPtr& new_task_item)
            {
                if (!new_task_item)
                    return false;
                ...
                {
                    writeLock locker(m_tasks_mtx);
                    auto iter = m_wait_props.find(new_task_item->get_prop_type());
                    if (iter == m_wait_props.end())
                    {
                        m_wait_props[new_task_item->get_prop_type()] = m_wait_tasks.size();
        ...
        private:
            ...
            // 总待执行任务队列,<属性,该属性在m_wait_tasks的位置(0为基数)>
            std::map<PropType, size_t>  m_wait_props;
            // 最大任务个数,当为0时表示无限制
...
#pragma endregion
    ...
    private:
        ...
        void create_thread(size_t thread_num)
        {
            ...
            {
                SafeThread* thd = new SafeThread(std::bind(&LastTaskPool<PropType>::thread_fun, this, m_cur_thread_ver));
