...
namespace BTool
{
    namespace BoostNet
    {
        ...
        class WebsocketSession : public std::enable_shared_from_this<WebsocketSession<IsBinary, IsReadSome>>
        {
        public:
            ...
            typedef BoostNet::WriteBuffer<1024>                                     WriteBufferType;
        ...
        public:
            ...
            void connect(const char* host, unsigned short port, char const* addr = "/") {
                ...
                m_resolver.async_resolve(host, std::to_string(port).c_str(),
                    boost::beast::bind_front_handler(&WebsocketSessionType::handle_resolve, this->shared_from_this()));
            }
            ...
            void start() {
                ...
                m_socket.async_accept_ex(
                    ...
                    boost::beast::bind_front_handler(&WebsocketSessionType::handle_start, this->shared_from_this()));
#else
                m_socket.async_accept(boost::beast::bind_front_handler(&WebsocketSessionType::handle_start, this->shared_from_this()));
#endif
            }

            // 同步关闭
            void shutdown(const boost::beast::error_code& ec = boost::asio::error::operation_aborted) {
        ...
        private:
            ...
            bool read() {
                //try {
                    if constexpr (IsReadSome) {
                        m_socket.async_read_some(m_read_buf.prepare(m_max_rbuffer_size),
                            boost::beast::bind_front_handler(&WebsocketSessionType::handle_read, this->shared_from_this()));
                    }
                    else {
                        m_socket.async_read(m_read_buf.streambuf(),
                            boost::beast::bind_front_handler(&WebsocketSessionType::handle_read, this->shared_from_this()));
                    }
                    return true;
                // }
            ...
            void write() {
                ...
                if constexpr (IsReadSome) {
                    m_socket.async_write(boost::asio::buffer(m_current_send_msg->data(), m_current_send_msg->size()), boost::beast::bind_front_handler(&WebsocketSessionType::handle_write, this->shared_from_this()));
                }
                // else {
                //     m_socket.write(boost::asio::buffer(m_current_send_msg->data(), m_current_send_msg->size()), boost::beast::bind_front_handler(&WebsocketSessionType::handle_write, this->shared_from_this()));
                // }
            }
            ...
            void handle_resolve(const boost::beast::error_code& ec, const boost::asio::ip::tcp::resolver::results_type& results) {
                ...
                boost::beast::get_lowest_layer(m_socket).async_connect(results,
                    boost::beast::bind_front_handler(&WebsocketSessionType::handle_connect, this->shared_from_this()));
            }
            ...
            void handle_connect(const boost::beast::error_code& ec, const boost::asio::ip::tcp::resolver::results_type::endpoint_type& end_point)
            {
                ...
                m_socket.async_handshake(m_connect_ip, m_hand_addr,
                    boost::beast::bind_front_handler(&WebsocketSessionType::handle_handshake, this->shared_from_this()));
            }
            ...
            // 处理读回调
            void handle_read(const boost::beast::error_code& ec, size_t bytes_transferred) {
                if (ec) {
                    shutdown(ec);
                    return;
                }
