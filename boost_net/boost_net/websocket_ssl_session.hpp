...
namespace BTool
{
    namespace BoostNet
    {
        ...
        class WebsocketSslSession : public std::enable_shared_from_this<WebsocketSslSession<IsBinary, IsReadSome>>
        {
        public:
        ...
        public:
            ...
            void connect(const char* host, unsigned short port, char const* addr = "/") {
                ...
                m_resolver.async_resolve(host, std::to_string(port).c_str(),
                    boost::beast::bind_front_handler(&WebsocketSslSessionType::handle_resolve, this->shared_from_this()));
            }
                ...
                m_socket.next_layer().async_handshake( boost::asio::ssl::stream_base::server,
                    [&](const boost::beast::error_code& ec) {
                        ...
                        m_socket.async_accept_ex(
                            ...
                            boost::beast::bind_front_handler(&WebsocketSslSessionType::handle_start, this->shared_from_this()));
#else
        ...
        private:
            ...
            bool read() {
                //try {
                    if constexpr (IsReadSome) {
                        ...
                        std::bind(&WebsocketSslSessionType::handle_read, this->shared_from_this(),
                            std::placeholders::_1, std::placeholders::_2));
                    }
                    else {
                        m_socket.async_read(m_read_buf.prepare(m_max_rbuffer_size),
                        std::bind(&WebsocketSslSessionType::handle_read, this->shared_from_this(),
                            std::placeholders::_1, std::placeholders::_2));
                    }
                    return true;
            ...
            void write() {
                ...
                if constexpr (IsReadSome) {
                    m_socket.async_write(boost::asio::buffer(m_current_send_msg->data(), m_current_send_msg->size()), boost::beast::bind_front_handler(&WebsocketSslSessionType::handle_write, this->shared_from_this()));
                }
                // else {
                //     m_socket.write(boost::asio::buffer(m_current_send_msg->data(), m_current_send_msg->size()), boost::beast::bind_front_handler(&WebsocketSslSessionType::handle_write, this->shared_from_this()));
                // }
            }
            ...
            void handle_resolve(const boost::beast::error_code& ec, const boost::asio::ip::tcp::resolver::results_type& results) {
                ...
                boost::beast::get_lowest_layer(m_socket).async_connect(results, 
                    boost::beast::bind_front_handler(&WebsocketSslSessionType::handle_connect, this->shared_from_this()));
            }
            ...
            void handle_connect(const boost::beast::error_code& ec, boost::asio::ip::tcp::resolver::results_type::endpoint_type)
            {
                ...
                m_socket.next_layer().async_handshake(
                    ...
                    boost::beast::bind_front_handler(
                        &WebsocketSslSessionType::handle_handshake,
