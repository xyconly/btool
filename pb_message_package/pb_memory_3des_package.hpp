...
namespace NPPb3DESMemoryPackage
{
#pragma pack(push)
#pragma pack(2) 
    // 包头
...
#pragma pack(pop)
    ...
    {
    public:
        EncodePackage(uint16_t command, const char* send_msg, size_t len, size_t pb_len)
            : m_head({ htons(command),htonl((uint32_t)len),htonl((uint32_t)pb_len) })
        ...
        char* reset_package() {
            char* send_msg = m_send_msg;
            m_send_msg = new char[0];
            m_send_len = 0;
            return send_msg;
        }
